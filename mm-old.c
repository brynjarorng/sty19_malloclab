...
void mm_free(void *bp)
{
    ...
    PUT(bp + DSIZE, PREV_BLKP(heap_listp));

    // epilogue->prev->next = freeblock
    PUT(PREV_BLKP(heap_listp) + WSIZE, bp);
...
static void *coalesce(void *bp) 
{
    ...
    if (prev_alloc && next_alloc) {            // Case 1 - Nothing
        ...
        // create a pointer to the block below
        char*  belowPtr = bp + size;
        size_t below_size = GET_SIZE(belowPtr);

        // fix header and footer
        PUT(belowPtr - below_size, PACK(size + below_size, 0));
        PUT(belowPtr + size - WSIZE, PACK(size + below_size, 0));

        // remove below block from free list
        // prev->next = below_next
        PUT(PREV_BLKP(belowPtr) + WSIZE, *((size_t *)(belowPtr + WSIZE)));

        // next->prev = below_prev
        PUT(NEXT_BLKP(belowPtr) + DSIZE, *((size_t *)(belowPtr + DSIZE)));
    }/*
    else {                                     // Case 4 - Both
